...
class ShiftClient:
    ...
    def auto_retry_login(self):
        """Automatically retry login if session expired"""
        if self.login_retry_count >= self.max_login_retries:
            _L.error("Max login retries reached. Please check credentials.")
            return False

        _L.info("Session expired, attempting to re-login...")
        ...
        result = self.__login(user, pw)
        if result and self.__save_cookie():
            _L.info("Re-login successful")
            self.login_retry_count = 0
            return True
        return False
    ...
    def redeem(self, code: str, game: str, platform: str) -> Status:
        ...
        if not found:
            status = enhanced_error_detection(str(form_data), status_code)

            # Auto-retry for session issues
            if status == Status.LOGIN_REQUIRED and self.auto_retry_login():
                return self.redeem(code, game, platform)

        else:
            status = self.__redeem_form(cast(dict[str, str], form_data))

            # Auto-retry for session issues during redemption
            if status == Status.LOGIN_REQUIRED and self.auto_retry_login():
                return self.redeem(code, game, platform)
